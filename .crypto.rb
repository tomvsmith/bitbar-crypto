require "rubygems"
require "json"

class Crypto
  def initialize(symbol = :eth, source = :bitfinex, decimals = 0)
    throw ArgumentError("Invalid Symbol") if ![:eth, :btc, :bch, :cred, :gold].include? symbol
    throw ArgumentError("Invalid Source") if ![:bitfinex, :kraken, :bitstamp, :cryptocompare, :coinmarketcap, :bullionvault].include? source

    @@symbol = symbol
    @@source = source
    @@decimals = decimals
  end

  def refresh
    format(latest_price)
  end

  def logo
    case @@symbol
    when :eth
      ""
    when :btc
      ""
    when :bch
      ""
    when :cred
      ""
    when :gold
      ""
    end
  end

  private
  def latest_price
    cur = @@symbol.to_s.upcase # :eth => "ETH"

    case @@source
    when :kraken
      res = %x(curl -s "https://api.kraken.com/0/public/Ticker?a=fetch&pair=#{cur}USD")
      res_obj = JSON.parse(res)
      price = res_obj['result']["X#{cur}ZUSD"]['a'][0]
    when :bitfinex
      res = %x(curl -s "https://api.bitfinex.com/v1/pubticker/#{cur}usd")
      res_obj = JSON.parse(res)
      price = res_obj['ask']
    when :bitstamp
      res = %x(curl -s "https://www.bitstamp.net/api/v2/ticker/#{cur}usd/")
      res_obj = JSON.parse(res)
      price = res_obj['ask']
    when :cryptocompare
      res = %x(curl -s "https://min-api.cryptocompare.com/data/price?fsym=#{cur}&tsyms=USD")
      res_obj = JSON.parse(res)
      price = res_obj['USD']
    when :coinmarketcap
      cur = "verify" if @@symbol == :cred
      res = %x(curl -s "https://api.coinmarketcap.com/v1/ticker/#{cur}/")
      res_obj = JSON.parse(res)
      price = res_obj[0]['price_usd']
    when :bullionvault
      res = %x(curl -s "https://www.galmarley.com/prices/prices.json?noCache=#{Time.now.to_i}&version=v2&chartType=CHART_POINTS&securityId=AUX&valuationSecurityId=USD&interval=3600&batch=Full&_=#{Time.now.to_i}")
      res_obj = JSON.parse(res)
      price = res_obj['latestPrice']['price']
    end

    price # str
  end

  def format(amount)
    "$" + separate_comma(amount)
  end

  def separate_comma(number)
    whole, decimal = number.to_s.split(".")
    whole_with_commas = whole.chars.to_a.reverse.each_slice(3).map(&:join).join(",").reverse
    whole_with_commas += "." + decimal[0, @@decimals] if @@decimals > 0

    whole_with_commas
  end
end
